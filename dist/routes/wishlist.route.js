...
router.get("/", auth_middleware_1.isAuthenticated, async (req, res) => {
    try {
        ...
        if (!id || !Array.isArray(id)) {
            return res
                .status(400)
                .json({ message: "Invalid input! ids must be an array of numbers." });
        }
        const numericIds = id.map((id) => Number(id)).filter((id) => !isNaN(id));
        if (numericIds.length === 0) {
            return res
                .status(400)
                .json({ message: "Invalid input! Array must contain valid numbers." });
        }
...
router.get("/id", auth_middleware_1.isAuthenticated, async (req, res) => {
    try {
        ...
        const products = await product_model_1.default.find({ id: { $in: numericIds } }, { price: 1, stock: 1, _id: 0, id: 1 });
        if (products.length === 0) {
            return res.status(404).json({ message: "No products found!" });
        }
        res.status(200).json(products);
